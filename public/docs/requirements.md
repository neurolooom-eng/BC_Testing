...
| REQ-OPX-017 | Before saving a slot in the Operator view, where one or more earlier slots after the latest recorded reading have no reading, the system shall name those slots, state that saving will lock them, and save only on a further explicit confirmation. | Saving a slot locks all earlier slots; a mis-tap on the slot strip must not silently lock empty ones. | Test | Implemented |
