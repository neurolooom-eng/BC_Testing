...
| REQ-OPX-013 | On signing in, a user who may record hourly readings but may not view Configuration shall be taken directly to the open day sheet for the current production day when exactly one such sheet exists, and to the dashboard otherwise. | An operator signs in to record a reading, not to navigate — but with several lines or furnaces a guessed sheet risks readings on the wrong one. | Test | Implemented |
| REQ-OPX-014 | The dashboard shall present, to any user who may record hourly readings, one Quick Record card per open day sheet for the current production day, each naming its line and furnace; where none exists, a single card for the most recent open day sheet. | The same shortcut, for users who are not redirected into it, with the line and furnace chosen rather than guessed. | Test | Implemented |
...
| REQ-OPX-017 | Before saving hourly readings in any hourly view, where one or more earlier slots after the latest recorded reading would remain without a reading, the system shall name those slots, state that saving will lock them, and save only on a further explicit confirmation. Slots filled in the same save shall count as filled. | Saving a slot locks all earlier slots; no view may lock an empty one silently. | Test | Implemented |
...
| REQ-OFF-007 | Where the network has not answered a request within 4 seconds and a cached copy of that request exists, the service worker shall serve the cached copy. | A connected-but-dead network can leave a request pending far longer than a network failure; the operator should not wait on it. | Test | Implemented |
| REQ-OFF-008 | When a page that has not been cached is requested with no network connection, the system shall display a notice naming that page as unavailable offline, with a link to the Process Check Sheet. | The operator must not be shown one page under another page's address. | Test | Implemented |
| REQ-OFF-009 | Each deployment shall replace the service worker's cached application shell in full, and remove the previous deployment's cache. | Offline must never combine files from two deployments. | Test | Implemented |
...
| REQ-OFF-010 | The deployment process shall verify that the production-day start used by Quick Record equals the one used by the Process Check Sheet. | The value is duplicated because the sign-in and dashboard pages do not load the Process Check Sheet modules; a drift would resolve the wrong day's sheet. | Test | Implemented |
