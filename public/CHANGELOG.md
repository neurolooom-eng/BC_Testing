...
- Every form and modal in the Process Check Sheet now has an **Auto-fill**
  button that populates the visible inputs with generated values, leaving
  the record unsaved so the operator can review and adjust before pressing
  Save
- Covered forms: new day sheet, day details edit, machine modal, shift
  detail modal, sign-off modal, hourly form view, and hourly matrix view
- Values are generated by the same engine as the direct-to-storage
  generators (v1.8.0), so they follow the acceptance limits and respect
  the selected mode (in-spec / occasional out of spec)
- Die Temp inputs in the hourly form are filled for running machines only;
  idle machines are left as NA
- Live validation fires as the values are filled, so out-of-spec
  highlighting appears immediately
- Auto-fill buttons are gated behind the same `action.pcs.demo.fill`
  permission as the existing test data controls
- The existing direct-to-storage "Fill" buttons remain for bulk operations
